...
assert.ok(['direct', 'restored', 'application', 'incomplete'].includes(mode), 'Unknown case');
...
if (mode === 'incomplete') {
  verify.incomplete(path.join(root, mode));
  console.log('PASS: incomplete lineage with successful remote job ' + jobId);
  process.exit(0);
}
